...
class _ChatPageState extends State<ChatPage> {
  ...
  Widget _buildImagePreview(String? base64Image) {
    ...
    try {
      ...
      return Container(width: 100, height: 100, color: Colors.grey.shade800,
  ...
  Future<void> _deleteMessage(int messageId) async {
    try {
      await http.post(
        Uri.parse('https://listo4ek.tech/delete_meme.php'),
      ...
      body: Column(
        children: [
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text(_isLoading ? 'Loading...' : 'No messages yet', style: TextStyle(color: Colors.grey.shade600)),
                        if (!_isLoading) ...[
                          const SizedBox(height: 8),
                          Text('Send a message to start the conversation!', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                        ],
                      ],
                    ),
                  )
          ...
          Container(
            ...
            child: Row(
              children: [
                IconButton(onPressed: _isLoading ? null : _pickImage, icon: const Icon(Icons.image), tooltip: 'Attach image'),
                Expanded(
                  child: TextField(
